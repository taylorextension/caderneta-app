...
function tlv(id: string, value: string): string {
  ...
  if (len > 99) {
    throw new Error(
      `Campo ${id} excede 99 bytes (len=${len}). Reduza o conteúdo.`
    )
  }
...
function sanitizeMerchantText(input: string, maxLen: number): string {
  const noAccents = input.normalize('NFD').replace(/[\u0300-\u036f]/g, '') // remove diacríticos
...
function formatAmount(amount: number): string {
  ...
  if (s.length > 13)
    throw new Error('valor muito grande para o campo 54 (max 13 chars).')
  return s
}
...
export function gerarBRCode(p: PixParams): string {
  ...
  const maiChildren = [tlv('00', 'br.gov.bcb.pix'), tlv('01', pixKey)]
