    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [statsRes, usersRes] = await Promise.all([
                ...
                fetch(`/api/admin/users?filter=${filter}&search=${encodeURIComponent(search)}`),
            ])
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
                        {filters.map((f) => (
                            <button
                                ...
                                className={`px-3.5 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${filter === f.key
                            ...
                            <div key={user.id} className="bg-white rounded-xl p-4 space-y-3">
                                ...
                                <div className="flex items-center justify-between text-xs text-[#6B7280]">
                                    <div className="flex gap-3">
                                        <span>Plano: <strong className="text-[#02090A]">{user.plano || 'trial'}</strong></span>
                                        <span>
                                ...
                                <div className="flex gap-2 pt-1">
                                    <button
                                        ...
                                        className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-colors ${user.assinatura_ativa
